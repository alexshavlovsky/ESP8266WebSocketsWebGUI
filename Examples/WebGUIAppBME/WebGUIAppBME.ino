...
const char APP_JS[] PROGMEM = "application/javascript";
...
#include <BME280BRZO_I2C.h> //https://github.com/alexshavlovsky/BME280.git
BME280BRZO_I2C bme;

#include <FilterAVG.h>
FilterAVG <float, int32_t, int32_t, 100> BMEt(50, 200);
FilterAVG <float, int32_t, int32_t, 100> BMEh(50, 200);
FilterAVG <float, int32_t, int32_t, 100> BMEp(50, 200);
...
// unit: B000 = Pa, B001 = hPa, B010 = Hg, B011 = atm, B100 = bar, B101 = torr, B110 = N/m^2, B111 = psi
   if (enBme) {
    bme.read(BMEpres, BMEtemp, BMEhum, true, 1);
    BMEt.AddMeasure(BMEtemp);
    BMEp.AddMeasure(BMEpres);
    BMEh.AddMeasure(BMEhum);
   }
}
...
MenuItem * Menu1[] = {
    ...
    new MenuOutputInteger(13, "BME280 sensor found", "", []()->int32_t{return enBme;}, 1000),
    ...
    new MenuFolder(1, 40, "BME280 sensor"),
    new MenuOutputChart(40, "Temperature", "&deg;C", []()->float{return BMEt.GetAVGfloat();}, 200),
    new MenuOutputChart(40, "Humidity", " %", []()->float{return BMEh.GetAVGfloat();}, 200),
    new MenuOutputChart(40, "Pressure", " hPa", []()->float{return BMEp.GetAVGfloat();}, 200),    
};
...
void Task_RefreshI2CDevices() {
    ...
    if(BMEoff && enBme) {
      bme.begin(I2C_SDA_PIN,I2C_SCL_PIN);
    }
}
...
void setup() {
    ...
    tasks.setInterval(200,Task_GetBME); // get data from BME280 sensor

    WsServer.onEvent(WsDispatcher); // server side WS dispatcher
    HttpServer.on("/", HTTP_GET, []() {
        HttpServer.send_P(200, TEXT_HTML, HTML_TEMPLATE); //front-end app
    });
    HttpServer.on("/smoothie.js", HTTP_GET, []() {
        HttpServer.send_P(200, APP_JS, SMOOTHIE_JS); //front-end app
    });
